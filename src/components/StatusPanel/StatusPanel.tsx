  ...
  return (
    <section className="status-panel" aria-label="Backup and Restore Status">
      ...
      <div className="status-panel__log" role="log" aria-live="polite">
        {logs.length === 0 ? (
          <p className="status-panel__log-empty">
            No activity yet. Connect both accounts and start a backup or
            restore.
          </p>
        ) : (
      ...
      <div className="status-panel__actions">
        <button
          className="status-panel__btn status-panel__btn--backup"
          disabled={isBackupDisabled}
        >
          ⬇ Backup
        </button>
        <button
          className="status-panel__btn status-panel__btn--restore"
