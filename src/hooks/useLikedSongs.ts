 ...
 * Auto-fetches when account connects, exposes progress, clears on logout.
 */
  ...
  const fetchSongs = useCallback(async () => {
    ...
    try {
      ...
      if (msg.includes("fetch_liked_401"))
        ...
        setError(
          'Permission denied. Make sure "user-library-read" was granted during login.',
        );
      else if (msg.includes("fetch_liked_429"))
        setError(
          "Spotify rate limit hit. Wait at least 60 seconds, then click Try again once.",
        );
      else setError("Failed to load liked songs. Check your connection.");
      setStatus("error");
    } finally {
      setProgress(null);
    }
