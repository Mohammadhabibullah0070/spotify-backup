/**
 * usePlaylistCreator — creates empty playlists on destination account.
 * Writes sourceId → destId PlaylistMap to BackupContext for track restore.
 */
  ...
  const startCreating = useCallback(async () => {
    ...
    for (let i = 0; i < toCreate.length; i++) {
      ...
      try {
        ...
        failed++;
        addLog(
          `✗ Failed to create "${pl.name}": ${friendlyError(msg)}`,
          "error",
        );
    ...
    addLog(
      `✓ Playlist creation complete (${created} created, ${failed} failed)`,
      "success",
    );
...
function friendlyError(msg: string): string {
  ...
  if (code === "create_playlist_401")
    return detail || "Session expired — reconnect destination";
  if (code === "create_playlist_403")
    return detail || "Permission denied — check playlist-modify scopes";
  if (code === "create_playlist_429")
    return "Rate limited by Spotify — wait a minute and retry";
